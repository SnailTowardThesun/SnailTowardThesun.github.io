...
这道题展示了一个经典的算法优化思想：**数学推导 + 递推关系**。在实际开发中，这种思想有很多应用场景：

1. **财务计算**：计算股票投资组合的加权收益、贷款利息的复利计算等
2. **信号处理**：数字信号处理中的快速傅里叶变换（FFT）就是利用了数学推导来优化计算复杂度
3. **机器学习**：梯度下降算法中的反向传播，也是利用链式法则的数学推导来高效计算梯度
4. **物理模拟**：天体运动模拟、粒子系统更新等，都需要利用数学公式来递推计算

这道题从O(n²)的暴力解法优化到O(n)的递推解法，完美展现了数学的力量！
